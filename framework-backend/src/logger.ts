...
export function logUser(content: string) {
    session.entries.push({ role: 'user', content, timestamp: new Date().toISOString() })
}

export function logAssistant(content: string) {
    session.entries.push({ role: 'assistant', content, timestamp: new Date().toISOString() })
}

export function logTool(content: string) {
    session.entries.push({ role: 'tool', content, timestamp: new Date().toISOString() })
}
...
export function saveSession() {
    ...
    const filename  = `${timestamp}_${session.scenario}.md`
